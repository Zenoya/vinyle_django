...
from .forms import ContactForm, ParagrapheErrorList
...
def listing(request):
    albums_list = Album.objects.filter(available=True).order_by('-id')
...
def detail(request, album_id):
    ...
    if request.method == 'POST':
        form = ContactForm(request.POST, error_class=ParagrapheErrorList)
        if form.is_valid():
